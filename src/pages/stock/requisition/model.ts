...
import { stockAndrequisitionApi } from './service';
...
export type ConnectPageState = ConnectState & { stockAndrequisition: PartialStateType };
...
const Model: ModelType = {
  ...
  effects: {
    *fetchList({ payload }, { call, put }) {
      const res = yield call(stockAndrequisitionApi.index, payload);
      yield put({ type: 'saveList', payload: res && Array.isArray(res.data) ? res.data : [] });
      return res;
    },

    *fetchItem({ payload }, { call, put }) {
      const res = yield call(stockAndrequisitionApi.byId, payload);
      yield put({ type: 'saveItem', payload: res && Array.isArray(res.data) ? res.data : [] });
      return res;
    },

    *addItem({ payload }, { call }) {
      const res = yield call(stockAndrequisitionApi.add, payload);
      return !!res;
    },

    *updateItem({ payload }, { call }) {
      const res = yield call(stockAndrequisitionApi.update, payload);
      return !!res;
    },

    *deleteItem({ payload }, { call }) {
      const res = yield call(stockAndrequisitionApi.destroy, payload);
      return !!res;
    },
    ...
    *switchStatus({ payload }, { call, put }) {
      ...
      const res = yield call(
        enabled ? stockAndrequisitionApi.disable : stockAndrequisitionApi.enable,
        id,
      );
