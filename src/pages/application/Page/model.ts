...
import { applicationList, applicationAdd, applicationUpdate } from './service';
...
export type ConnectPageState = ConnectState & { applicationAndPage: PartialStateType };
...
const Model: ModelType = {
  ...
  effects: {
    *fetchList({ payload }, { call, put }) {
      const res = yield call(applicationList.isApi, payload);
      yield put({
        type: 'saveList',
    ...
    *fetchItem({ payload }, { call, put }) {
      const res = yield call(applicationList.byId, payload);
    ...
    *addItem({ payload }, { call }) {
      ...
      const res = yield call(applicationAdd, payload);
      return !!res;
    },
    ...
    *updateItem({ payload }, { call }) {
      ...
      const res = yield call(applicationUpdate, payload);
      return !!res;
    },

    *deleteItem({ payload }, { call }) {
      const res = yield call(applicationList.destroy, payload);
      return !!res;
    },

    *switchStatus({ payload }, { call, put }) {
      const { enabled, id } = payload;
      const res = yield call(enabled ? applicationList.disable : applicationList.enable, id);
