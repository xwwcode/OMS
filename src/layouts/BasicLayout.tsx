...
export interface BasicLayoutProps extends ProLayoutProps {
  ...
  settings: Settings;
  dispatch: Dispatch;
  menuData: ConnectState['login']['menuData'];
}
  ...
  const menuDataRender = useCallback(
    (menuList: MenuDataItem[]): MenuDataItem[] => {
      const menuListLoopMerge = (
        ...
        list.forEach(item => {
          if (!item.hideInMenu) {
            const userMenuItem = userMenu.find(n => !!n.url && n.url === item.path);
            if (userMenuItem) {
              arr.push({
                ...item,
                ...userMenuItem,
                children:
                  item.children && userMenuItem.children
                    ? menuListLoopMerge(item.children, userMenuItem.children)
                    : [],
              });
            }
          }
      ...
      const menus = menuListLoopMerge(menuList, menuData);
      return menus;
    },
    [menuData],
  );
